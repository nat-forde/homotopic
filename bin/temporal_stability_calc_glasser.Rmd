...
ggplot(simplified_pond, aes(x=sex, y=QC.resid , color = NDD)) + 
  ...
  facet_wrap(~FCtype)
```
## statistics STABILITY - groupwise
```{r, fig.width=12}
...
print(paste("Anova Table for fctype X NDD"))
a1 <- aov(Z ~ FCtype:NDD + FCtype:ABAS_GC +NDD:ABAS_GC +FCtype +NDD + ABAS_GC + Age + sex + Comp.1 + Comp.2, 
               data = simplified_pond)
print(summary(a1))
print(TukeyHSD(x=a1, 'NDD', conf.level=0.95))
print(TukeyHSD(x=a1, 'FCtype', conf.level=0.95))
