  ...
  for (i in 1:nrow(theZs)) {
    ...
    if (file.exists(meants.file)) {
      ...
      # take the egde list as a vector
        ...
        edges.dw_norm <- merge(edges.dw_w, len.df[c(1,2,8)], by=c('V1', 'V2'))
       ...
       #can merge to get full edge list again
        edges.dw_all <- merge(edges.dw_norm, edges.dw_nonzero[c(1,2,12,13)], by=c('V1', 'V2'), all.x=TRUE)
        
        edges.ZTS <- merge(edges.Z, edges.TS, by=c('V1', 'V2', 'V1.hemi', 'V1.roi', 'V2.hemi', 'V2.roi', 'FCtype'), all.x=TRUE)
        edges.all <- merge(edges.ZTS, edges.dw_all, by=c('V1', 'V2', 'V1.hemi', 'V1.roi', 'V2.hemi', 'V2.roi', 'FCtype'), all.x=TRUE)
...
type_ <- merge(typeZ[c(1,44:48,160:165,177,178)], typeTS[c(1,177,178)], by=c("subid","FCtype"))  #index numbers may change if QC added
type <- merge(type_, typeDW[c(1,177,178)], by=c("subid","FCtype")) #%>%
  #filter(NDD != "")
...
lenQ_ <- merge(lenQZ[c(1,27:31,143:148,160,161)], lenQTS[c(1,160,161)], by=c("subid","lenQ"))  #index numbers may change if QC added
lenQ  <- merge(lenQ_, lenQDW[c(1,160,161)], by=c("subid","lenQ")) #%>%
  #filter(NDD != "")
...
dwQ_ <- merge(dwQZ[c(1,27:31,143:148,160,161)], dwQTS[c(1,160,161)], by=c("subid","dwQ"))  #index numbers may change if QC added
dwQ  <- merge(dwQ_, dwQDW[c(1,160,161)], by=c("subid","dwQ"))# %>%
  #filter(NDD != "")
...
F2 <- lmer(TS ~ FCtype*NDD + Age + sex + (1|subid), 
               data = type)
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
summary(glht(F2, linfct=mcp(FCtype ="Tukey")))
...
# type x dx (male only)
print(paste("Anova Table for type X NDD, male only"))
F2 <- lmer(TS ~ FCtype*NDD + Age + (1|subid), 
               data = filter(type, sex == "Male"))
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
summary(glht(F2, linfct=mcp(FCtype ="Tukey")))
...
# plot lenQ X dx 
ggplot(lenQ, aes(x=lenQordered, y=Z, color = NDD)) + 
  geom_boxplot() 
...
ggplot(filter(lenQ, sex == "Male"), aes(x=lenQordered, y=Z, color = NDD)) + 
  geom_boxplot() 
...
F2 <- lmer(TS ~ lenQ*NDD + Age + sex + (1|subid), 
               data = lenQ)
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
#summary(glht(F2, linfct=mcp(lenQ ="Tukey")))
...
# plot lenQ X dx 
ggplot(lenQ, aes(x=lenQordered, y=TS, color = NDD)) + 
  geom_boxplot() 
...
# lenQ x dx (male only)
print(paste("Anova Table for lenQ X NDD, male only"))
F2 <- lmer(TS ~ lenQ*NDD + Age + (1|subid), 
               data = filter(lenQ, sex == "Male"))
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
#summary(glht(F2, linfct=mcp(lenQ ="Tukey")))
...
# plot lenQ X dx (male only)
ggplot(filter(lenQ, sex == "Male"), aes(x=lenQordered, y=TS, color = NDD)) + 
  geom_boxplot() 
...
# plot lenQ X dx 
ggplot(lenQ, aes(x=lenQordered, y=DW, color = NDD)) + 
  geom_boxplot() 
...
# plot lenQ X dx (male only)
ggplot(filter(lenQ, sex == "Male"), aes(x=lenQordered, y=DW, color = NDD)) + 
  geom_boxplot() 
...
F2 <- lmer(Z ~ dwQ*NDD + Age + sex + (1|subid), 
               data = dwQ)
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
...
# plot dwQ X dx 
ggplot(dwQ, aes(x=dwQordered, y=Z, color = NDD)) + 
  geom_boxplot() 
...
ggplot(filter(dwQ, sex == "Male"), aes(x=dwQordered, y=Z, color = NDD)) + 
  geom_boxplot() 
...
F2 <- lmer(TS ~ dwQ*NDD + Age + sex + (1|subid), 
               data = dwQ)
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
#summary(glht(F2, linfct=mcp(dwQ ="Tukey")))
...
# plot dwQ X dx 
ggplot(dwQ, aes(x=dwQordered, y=TS, color = NDD)) + 
  geom_boxplot() 
...
# dwQ x dx (male only)
print(paste("Anova Table for dwQ X NDD, male only"))
F2 <- lmer(TS ~ dwQ*NDD + Age + (1|subid), 
               data = filter(dwQ, sex == "Male"))
print(Anova(F2))
summary(glht(F2, linfct=mcp(NDD ="Tukey")))
#summary(glht(F2, linfct=mcp(dwQ ="Tukey")))
...
# plot dwQ X dx (male only)
ggplot(filter(dwQ, sex == "Male"), aes(x=dwQordered, y=TS, color = NDD)) + 
  geom_boxplot() 
