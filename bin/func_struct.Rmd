...
type_ <- merge(typeZ[c(1,46:50,162:167,179,180)], typeTS[c(1,179,180)], by=c("subid","FCtype"))  #index numbers may change if QC added
type <- merge(type_, typeDW[c(1,179,180)], by=c("subid","FCtype")) #%>%
  #filter(NDD != "")
...
lenQ_ <- merge(lenQZ[c(1,28:32,144:149,161,162)], lenQTS[c(1,161,162)], by=c("subid","lenQ"))  #index numbers may change if QC added
lenQ  <- merge(lenQ_, lenQDW[c(1,161,162)], by=c("subid","lenQ")) #%>%
  #filter(NDD != "")
...
dwQ_ <- merge(dwQZ[c(1,28:32,144:149,161,162)], dwQTS[c(1,161,162)], by=c("subid","dwQ"))  #index numbers may change if QC added
dwQ  <- merge(dwQ_, dwQDW[c(1,161,162)], by=c("subid","dwQ"))# %>%
  #filter(NDD != "")
