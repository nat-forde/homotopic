...
```{r}
#compuls
comp_demographics <- read.csv("/projects/nforde/COMPULS/clinical/pheno_Nij.csv")
comp_QCfunc <- read.csv("/projects/nforde/COMPULS/data/bids/derivatives/ bold.csv")
...
```{r}
# filter to get rid of bad QC
comp_demogs <- comp_demogs %>% filter(fd_mean < 0.5) %>% filter(size_t > 100) ### dwi data filtered in analyses
...
# plot type X dx 
ggplot(filter(type_trio, cutree.res.hc..k...3. != 3), aes(x=FCtype, y=DW, color = dx)) + 
...
pltTrioDW_dx_male <- ggplot(filter(type_trio, cutree.res.hc..k...3. != 3, sex =="Male"), aes(x=FCtype, y=DW, color = dx)) + 
  geom_boxplot() +ylim(0,70)

pltTrioDW_dx_female <- ggplot(filter(type_trio, cutree.res.hc..k...3. != 3, sex == "Female"), aes(x=FCtype, y=DW, color = dx)) + 
  geom_boxplot() +ylim(0,70)
...
```{r TRIO-DWI-CBCL}
print(paste("Anova - Does tract type predict structure? - trio - GAF"))
F2.trio.DWcbcl <- lmer(DW ~ FCtype*CB68TPTOT*sex + Age + PC1 + (1|subid), 
               data = filter(type_trio, cutree.res.hc..k...3. != 3))
#summary(F2)
...
pltTrioDW_cbcl <-ggplot(filter(type_trio, cutree.res.hc..k...3. != 3), aes(x=CB68TPTOT, y=DW, color = FCtype)) + 
  geom_jitter(width = 0.3) +  geom_smooth(span = 0.5) +ylim(0,70) +xlim(0,160)
pltTrioDW_cbcl_male <-ggplot(filter(type_trio, cutree.res.hc..k...3. != 3, sex =="Male"), aes(x=CB68TPTOT, y=DW, color = FCtype)) + 
  geom_jitter(width = 0.3) +  geom_smooth(span = 0.5) +ylim(0,70) +xlim(0,160)
pltTrioDW_cbcl_female <-ggplot(filter(type_trio, cutree.res.hc..k...3. != 3, sex =="Female"), aes(x=CB68TPTOT, y=DW, color = FCtype)) + 
  geom_jitter(width = 0.3) +  geom_smooth(span = 0.5) +ylim(0,70) +xlim(0,160)
